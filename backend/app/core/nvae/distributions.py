...
from app.core.nvae.utils import one_hot
...
class DiscLogistic:
    ...
    def sample(self):
        u = torch.Tensor(self.means.size()).uniform_(1e-5, 1. - 1e-5)                       # B, 3, H, W
...
class DiscMixLogistic:
    ...
    def sample(self, t=1.):
        gumbel = -torch.log(- torch.log(torch.Tensor(self.logit_probs.size()).uniform_(1e-5, 1. - 1e-5)))  # B, M, H, W
        ...
        u = torch.Tensor(means.size()).uniform_(1e-5, 1. - 1e-5)                        # B, 3, H, W
