...
from app.core.nvae.neural_operations import OPS, EncCombinerCell, DecCombinerCell, Conv2D, get_skip_connection, SE
from app.core.nvae.neural_ar_operations import ARConv2d, ARInvertedResidual, MixLogCDFParam, mix_log_cdf_flow
from app.core.nvae.neural_ar_operations import ELUConv as ARELUConv
from torch.distributions.bernoulli import Bernoulli

from app.core.nvae.utils import get_stride_for_cell_type, get_input_size, groups_per_scale
from app.core.nvae.distributions import Normal, DiscMixLogistic
from app.core.nvae.thirdparty.inplaced_sync_batchnorm import SyncBatchNormSwish
...
class Cell(nn.Module):
    def __init__(self, Cin, Cout, cell_type, arch, use_se):
        ...
        for i in range(self._num_nodes):
            ...
            primitive = arch[i]
            # print("                 Primitive type: {}".format(primitive))
            # print("                 In: {}".format(C))
...
class AutoEncoder(nn.Module):
    def __init__(self, args, writer, arch_instance):
        ...
        # print("begin encoder tower")
        if self.vanilla_vae:
            self.enc_tower = []
        else:
            self.enc_tower, mult = self.init_encoder_tower(mult)
        # print("end encoder tower")
    ...
    def init_stem(self):
        ...
        # print("Stem conv. Cin {}. Cout {}.".format(Cin, Cout))
    ...
    def init_pre_process(self, mult):
        ...
        for b in range(self.num_preprocess_blocks):
            # print("Preprocess block: {}".format(b))
            for c in range(self.num_preprocess_cells):
                # print("     Preprocess cell: {}".format(c))
    ...
    def init_encoder_tower(self, mult):
        ...
        for s in range(self.num_latent_scales):
            # print("Scale: {}, mult: {}".format(s, mult))
    ...
    def init_normal_sampler(self, mult):
        ...
        for s in range(self.num_latent_scales):
            ...
            for g in range(self.groups_per_scale[self.num_latent_scales - s - 1]):
                ...
                num_c = int(self.num_channels_enc * mult)
                # print(self.num_channels_enc)
                # print("         encoder sampler conv. Cin {}. Cout {}.".format(num_c, 2 * self.num_latent_per_group))
    ...
    def init_post_process(self, mult):
        ...
        for b in range(self.num_postprocess_blocks):
            # print("Postprocess block: {}".format(b))
            for c in range(self.num_postprocess_cells):
                # print("     Postprocess cell: {}".format(c))
                if c == 0:
        ...
        for i in weights:
            ...
            with torch.no_grad():
                ...
                if i not in self.sr_u:
                    ...
                    self.sr_u[i] = F.normalize(torch.ones(num_w, row).normal_(0, 1), dim=1, eps=1e-3)
                    self.sr_v[i] = F.normalize(torch.ones(num_w, col).normal_(0, 1), dim=1, eps=1e-3)
